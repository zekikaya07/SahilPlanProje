...
namespace SahilPlanProje.WebApi.Controllers
{
    ...
    [ApiController]
    public class TahakkukScalesController : ControllerBase
    {
        private readonly ApiContext _context;
        private readonly IMapper _mapper;

        public TahakkukScalesController(ApiContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        {
            var values = _context.TahakkukScales.ToList();
            return Ok(_mapper.Map<List<ResultTahakkukScaleDto>>(values));
        }

        [HttpGet("{id}")]
        public IActionResult GetTahakkuk(int id)
        {
            var value = _context.TahakkukScales.Find(id);
            return Ok(_mapper.Map<GetByIdTahakkukScaleDto>(value));
        }

        [HttpPost]
        public async Task<IActionResult> CreateCity([FromBody] CreateTahakkukScaleDto city)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);

            var value = _mapper.Map<TahakkukScale>(city);

            await _context.TahakkukScales.AddAsync(value);
            ...
            return Ok(new
            {
                ...
                data = _mapper.Map<ResultTahakkukScaleDto>(value)
            });
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateCity(int id, [FromBody] UpdateTahakkukScaleDto city)
        {
            if (id != city.id)
                return BadRequest("Id uyuşmuyor.");

            var existingCity = await _context.TahakkukScales.FindAsync(id);
        ...
        {
            var city = await _context.TahakkukScales.FindAsync(id);

            if (city == null)
                return NotFound("Kayıt bulunamadı.");

            _context.TahakkukScales.Remove(city);
